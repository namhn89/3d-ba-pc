...
def draw_heatcloud(inpCloud, hitCheckArr, mode):
    ...
    pcd.points = open3d.utility.Vector3dVector(inpCloud[0])
    pcd.colors = open3d.utility.Vector3dVector(pColors)
...
def draw_NewHeatcloud(inputPCArray, inputWeightArray):
    ...
    pcd = open3d.geometry.PointCloud()
    pcd.points = open3d.utility.Vector3dVector(inputPCArray[0])
    pcd.colors = open3d.utility.Vector3dVector(pColors)
    open3d.visualization.draw_geometries([pcd])
...
def draw_pointcloud(inputPointCloudArr):
    pcd = open3d.geometry.PointCloud()
    pcd.points = open3d.utility.Vector3dVector(inputPointCloudArr[0])
    pcd.colors = open3d.utility.Vector3dVector(np.zeros((len(inputPointCloudArr[0]), 3), dtype=float))
    open3d.visualization.draw_geometries([pcd])
